...
namespace MinusMKI
{
	...
	public:
		...
		virtual void SetTime(double ts) {};
	...
	public:
		...
		constexpr static float normfreq = 1.0 / TableWidth;
	private:
	...
	public:
		void Apply(float* table, int numSamples) override
		{
			...
			for (int i = 1; i < half; ++i)
			{
				...
				x = sqrtf(x);
				float m = cosf(x * 2.0 * M_PI * comb);//这也很disperse
				m *= m * m;//niiiice!
				descTableRe[i] *= m;
				descTableIm[i] *= m;
			}
	...
	template<int TableWidth>
	class TableMutantVirusGrain :public TableMutant
	{
	private:
		double ts = 0;
		...
		float tmpRe[TableWidth] = { 0 };
		float tmpIm[TableWidth] = { 0 };
		float tmpRe2[TableWidth] = { 0 };
		float tmpIm2[TableWidth] = { 0 };
	...
	public:
		...
		float ProcessSampleHQ(float dt, float pm = 0.0)//低频模式
		{
			ProcessSwapTable();
		...
		float GetNowNaiveSample()//供pm用，直接取一个naive的相位
		{
			float fpos = t * TableWidth;
			int ipos = fpos;
			float frac = fpos - ipos;
			float magA1 = intMagtableA[ipos];
			float magB1 = intMagtableB[ipos];
			ipos += 1;
			if (ipos >= TableWidth)ipos = 0;
			float magA2 = intMagtableA[ipos];
			float magB2 = intMagtableB[ipos];
			float mag1 = magA1 + (magB1 - magA1) * sampleCounter;
			float mag2 = magA2 + (magB2 - magA2) * sampleCounter;
			float mag = mag1 + (mag2 - mag1) * frac;
			return mag * TableWidth;
		}

		float lastt = 0;
		float ProcessSampleLQ(float dt, float pm = 0.0)
		{
			ProcessSwapTable();
			dt += pm;
			if (dt > 0.499)dt = 0.499;
